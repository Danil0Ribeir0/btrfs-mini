...
std::expected<Arquivo, ErroDisco> Diretorio::criar_arquivo(const std::string& nome) {
    ...
    auto res_inode = arvore.inserir_item_tipado<Inode, 32>(chave_inode, inode_arquivo);
    ...
    auto res_dir = arvore.inserir_item_tipado<RegistroDiretorio, 64>(chave_dir, registro);
...
std::expected<Diretorio, ErroDisco> Diretorio::criar_diretorio(const std::string& nome) {
    ...
    ChaveBtrfs chave_inode{novo_id, BtrfsTipoItem::InodeItem, 0};
    auto res_inode = arvore.inserir_item_tipado<Inode, 32>(chave_inode, inode_dir);
    if (!res_inode) return std::unexpected(res_inode.error());
...
std::expected<std::vector<InfoEntrada>, ErroDisco> Diretorio::listar() {
    auto res_itens = arvore.listar_itens_tipado<RegistroDiretorio, 64>(this->id_inode, BtrfsTipoItem::ItemDiretorio);
